...
標題的標記方式為在行頭以\#註記，並在空行後輸入標題文字。\#字號的數量代表標題的層級，一般的Markdown標記一共提供有六層的標題記號。以下示範標註方式，並展示渲染後的範例。
...
透過以上的標題展示可以看到，第四級之後的標題文本漸漸變得比段落字體還小。因此，除非有特別調整渲染格式，一般Markdown文本在輸出為HTML時，文章結構不建議超過四級。
