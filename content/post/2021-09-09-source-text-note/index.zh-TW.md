...
或許一開始在學的時候會因為超級折騰，而放棄純文本回去用WYSIWYG。因此，在這一開始，我推薦你下先大略看看以下的精彩文章，參考其他人如何樂悠遊地運用純文本，並回過頭來對照自己的筆記及寫作流程，以及自己是否有使用標記語言進行純文本寫作的需求。但畢竟我也不能確定怎樣最好，因為我的筆記其實是綁在Zotero下的富文本格式（到時候要逃難可能也是極為困難的吧），但RStudio支持富文本的複製貼上，工作流程剛好也就無痛銜接了。
...
回過頭來，其實這篇文章只是要說明本站使用的技術，因此只是順勢推薦一下Markdown的入門資訊，並在這裡給一劑進入Markdown世界前的預防針，畢竟被卡車撞這種事情也不是每個人都會發生。最後，請善用[Duckduckgo](https://duckduckgo.com/)或[Startpage](https://www.startpage.com)等搜尋引擎（不推薦賣個資又養套殺使用者的Google）或各編輯器所提供的說明文件來學習，本文不再贅述。
...
一般的Markdown提供有六層的標題記號，但第五級之後的標題文字漸漸變得比段落字體還小，而且不會出現在預設生成的目錄表中。因此，除非有特別調整渲染格式，一般Markdown文本在輸出為HTML時，文章結構不建議超過四級。
