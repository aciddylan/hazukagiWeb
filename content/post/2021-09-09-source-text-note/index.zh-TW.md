...
如上列範例所示，註腳標註`[^13]`會顯示為含有數字13的上標。註腳的內容通常都會直接寫入註釋段落的下方，並在`[^13]:`後填入註釋的內容。本站及其他輸出為 HTML 頁面的情況時，顯示效果為將所有註腳集中在頁面的最下方，並提供雙向的跳轉連結；而在輸出為 `.pdf` 或 `.docx` 等文件時，則會出現在標註位置該頁面下方。
